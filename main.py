...
if __name__ == "__main__":
    ...
    parser.add_argument('model', choices=['ppo', 'dqn', 'qrdqn'], help='Name of agent to load/train.')
    ...
    if model == 'ppo':
        if args.train:
            df = ppo.train(verbose=True, dim=args.dim)
            if args.save_fig:
        ...
        if args.train:
            qrdqn.train(dim=args.dim)
        else:
            qrdqn.test(dim=args.dim)

    else:
        raise ValueError(f"Unknown model type: {model}. Please choose 'ppo', 'dqn' or 'qrdqn'.")
