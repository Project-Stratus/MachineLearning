...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--gpu",
        action="store_true",
        help="Use GPU for training/inference if available.",
    )
    parser.add_argument(
        "--hpc",
        action="store_true",
        help="HPC mode: disables progress bars for non-interactive SLURM jobs.",
    )
    parser.add_argument(
        "-d",
        "--dim",
        ...
        default=3,
        choices=[1, 2, 3],
        help="Dimensionality of the environment (1D, 2D or 3D).",
    )
    parser.add_argument(
        "-sf",
        "--save_fig",
        action="store_true",
        help="Save training figure to disk (only in train mode).",
    )
    parser.add_argument(
        "--n-envs",
        type=int,
        default=None,
        help="Number of parallel environments (overrides auto-detected default). Use to limit memory on HPC.",
    )
    parser.add_argument(
        "--balloon-type",
        type=str,
        default="zero_pressure",
    ...
    if args.benchmark:
        ...
        if args.save_fig:
            ...
            plt.grid()
            plt.savefig(
                f"{save_dir}/training_curve_dim{args.dim}_{args.balloon_type}.png"
            )
            plt.close()
            print(
                f"Training figure saved to {save_dir}/training_curve_dim{args.dim}_{args.balloon_type}.png"
            )
            ...
            if Path(eval_file).exists():
                ...
                plt.grid()
                plt.savefig(
                    f"{save_dir}/eval_curve_dim{args.dim}_{args.balloon_type}.png"
                )
                plt.close()
                print(
