...
DEFAULT_CSV = (
    Path(__file__).resolve().parents[1] / "EDA/Data/loon-flights-2021Q2.csv.gz"
)
...
def check_pressure(df: pd.DataFrame, atmo: Atmosphere) -> None:
    ...
    print(
        f"  ISA error (p5..p95)    {np.percentile(rel, 5):+.1f} .. {np.percentile(rel, 95):+.1f} %"
    )

    band = (alt >= ALT_SAFE_MIN) & (alt <= ALT_SAFE_MAX)
    if band.any():
        print(
            f"  -- within our operating band ({ALT_SAFE_MIN:,.0f}-{ALT_SAFE_MAX:,.0f} m), "
            f"n={band.sum():,}"
        )
...
def check_temperature(df: pd.DataFrame, atmo: Atmosphere) -> None:
    ...
    print(
        f"  measured T             {t_meas.min():.1f} - {t_meas.max():.1f} K "
    ...
    print(
        f"  ISA error (p5..p95)    {np.percentile(err, 5):+.1f} .. {np.percentile(err, 95):+.1f} K"
    )
    ...
    if band.any():
        ...
        print(
            f"     spread (p5..p95)    {np.percentile(t_meas[band], 5):.1f} .. "
...
def check_vertical_rates(df: pd.DataFrame) -> None:
    ...
    absv = np.abs(v)
    print(
        f"  samples                {len(v):,} across {df['flight_id'].nunique()} flights"
    )
    ...
    print(
        f"  -> clamp headroom      {VEL_MAX / max(absv.max(), 1e-9):.0f}x the fastest real sample"
    )
...
def check_horizontal_winds(df: pd.DataFrame) -> None:
    ...
    print(
        f"  -> {over:.2f} % of real samples exceed the normaliser (would clip to 1.0)"
    )
