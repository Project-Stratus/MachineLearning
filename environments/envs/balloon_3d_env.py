...
*   **Action space** `Discrete(3)`
    0→inflate,1→deflate,2→nothing (consistent across dimensions).
*   **Observation space** (all values are **scaled to [0,1]** for
    easy learning):

        {
            "goal":         (dim,),  # position of the goal point
            "volume":       (1,),
            "position":     (dim,),
            "velocity":     (dim,),
            "pressure":     (1,),   # ambient pressure at current altitude
            "wind":         (dim,)  # instantaneous wind vector at current pos
        }

    - For 1D we use z (altitude) only.
    - For 2D we use x (North-South) and y (East-West).
    - For 3D we use x, y, z.

    For x, positive is North
    For y, positive is East

*   **Reward** negative Euclidean distance to the goal, sharply
    penalised (-400) if the balloon hits the ground.
*   **Termination** balloon altitude ≤ 0 m (crash) or time limit
    reached.
*   **Visualisation**  Pygame window:

      - Left ¾  → top‑down *x‑y* (or *xz* in 2D) map with wind
        arrows.
      - Right ¼ → vertical altitude bar (like the original 1D view).

The class is designed to be highly configurable through the *config*
argument -pass any constant you see below to override it without
sub-classing.
"""
...
from environments.constants import VOL_MAX, ALT_MAX, VEL_MAX, P_MAX, DT
