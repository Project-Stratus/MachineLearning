...
MASS = (
    PAYLOAD_MASS + BALLAST_INITIAL
)  # Total structural mass (kg), used for ALT_MAX derivation
...
OSCILLATION_AMP = (
    0.05  # Breathing oscillation amplitude (fraction of stationary volume)
)
...
def _rho_effective(h: float) -> float:
    ...
    """
    p = _isa_pressure(h)
    return p * M_AIR / (R * _isa_temperature(h)) - p * M_HE / (
        R * _isa_gas_temperature(h)
    )
...
VENT_RATE_MOLES = (
    _P_FLOAT * VENT_RATE / (R * _T_GAS_FLOAT)
)  # mol removed per vent action
...
AIR_PUMP_RATE = (
    0.01  # Air mass pumped per action (kg) — equals BALLAST_DROP for force symmetry
)
AIR_BLADDER_MAX = 5.0  # Maximum air mass in bladder (kg)
AIR_BLADDER_INITIAL = (
    AIR_BLADDER_MAX / 2
)  # Start at midpoint for equal up/down authority (2.5 kg)
